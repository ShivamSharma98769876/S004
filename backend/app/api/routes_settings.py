...
DEFAULT_STRATEGY_DETAILS = {
    ...
    "indicators": {
        ...
        "emaCrossover": {"bonus": 1, "maxCandlesSinceCross": 3, "description": "Fast EMA crossed above slow EMA from lower to upper = +1 score bonus (bullish crossover). Freshness: cross within last 3 candles."},
    ...
    "strikeSelection": {
        ...
        "description": "Liquidity: min OI 10k, min volume 500. Max 3 steps OTM to reduce theta decay. Best strike: delta ~0.35 CE / -0.35 PE.",
    },
    "scoreThreshold": 3,
    "scoreMax": 6,
    "autoTradeScoreThreshold": 4,
    "scoreDescription": "Score 0-6: Primary(VWAP) + EMA + RSI + Volume + EMA crossover bonus + IVR bonus (when IVR<20). Crossover freshness: cross within 3 candles. ADX filter: no signals when ADX<25. Strike selection: max 3 steps OTM. Signal when score >= 3. Auto-trade when score >= 4.",
}
...
async def upsert_settings(payload: SettingsPayload, user_id: int = Depends(get_user_id)) -> dict[str, Any]:
    ...
    await execute(
        ...
        _parse_time(payload.strategy.get("tradeStart", "09:20")),
