...
Uses only stdlib math. Spot, strike, T (years), r (risk-free), LTP -> delta, theta, iv%.
"""
...
def call_theta(S: float, K: float, T: float, r: float, sigma: float) -> float:
    """Theta per year (decimal). Multiply by 1/365 for per-day if needed."""
    if T <= 0 or sigma <= 0:
        return 0.0
    d1 = _d1(S, K, T, r, sigma)
    d2 = _d2(S, K, T, r, sigma)
...
def iv_call_bisection(S: float, K: float, T: float, r: float, price: float, max_iter: int = 80) -> float:
    """Implied vol for call (0.001 to 3.0). Returns annual vol as decimal."""
    if T <= 0 or price <= 0:
        return 0.0
...
def time_to_expiry_years(expiry_date: date) -> float:
    """Years from today to expiry (fractional days)."""
    today = date.today()
    delta = (expiry_date - today).days
    if delta < 0:
        return 1e-4  # expired, avoid div by zero
    # Add fraction of day so same-day expiry is small but non-zero
...
def compute_greeks(
    ...
    """
    Returns (delta, theta_per_day, iv_pct).
    option_type is 'CE' or 'PE'. iv_pct is 0-100.
    """
    if spot <= 0 or ltp <= 0:
        return 0.0, 0.0, 0.0
    T = time_to_expiry_years(expiry_date)
    if T <= 0:
        T = 1e-4  # avoid div by zero
    if option_type == "CE":
    ...
    iv_pct = iv * 100.0
    return round(delta, 4), round(theta, 2), round(iv_pct, 2)
